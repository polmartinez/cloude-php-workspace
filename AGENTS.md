...
| Emit `CREATE TABLE` SQL | `Schema::createTableSql($table, $columns, $indexes, $foreignKeys, dialect: 'mysql')` | `Cloude\Storage\Schema` — standalone helper, not coupled to `Model`. Supports `mysql` / `pgsql`. **Not a migration framework** — feed the SQL to `phinx` / `doctrine/migrations` / `pdo->exec()` as you like |
| Foreign key with ON DELETE / ON UPDATE | `['columns' => ['role_id'], 'references' => 'roles', 'on' => ['id'], 'on_delete' => 'set null', 'on_update' => 'cascade']` | Inside the `$foreignKeys` arg of `Schema::createTableSql`. Actions: `cascade`, `set null`, `restrict`, `no action`, `set default` (case-insensitive) |
