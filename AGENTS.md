...
| Test base class with FW helpers | `class FooTest extends \Cloude\Testing\TestCase` — adds `useArrayModel()`, `useSqliteModel()`, `captureHttp()`, `assertJsonResponse()`, `assertHttpException()`, `freezeTime()`, `assertModelHas()`. Inherits from PHPUnit — all PHPUnit features still work |
