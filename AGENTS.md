...
| Declare indexes / FKs on the model | `protected static array $indexes / $foreignKeys` | Emit standalone SQL via `User::indexesSql()` / `User::foreignKeysSql()` — `CREATE INDEX` and `ALTER TABLE ... ADD CONSTRAINT`. Columns themselves live in your migrations |
| Emit `CREATE TABLE` SQL (full) | `Schema::createTableSql($table, $columns, $indexes, $foreignKeys, dialect: 'mysql')` | Standalone helper. `mysql` / `pgsql`. **Not a migration framework** — feed the SQL to `phinx` / `doctrine/migrations` / `pdo->exec()` as you like |
| Foreign key with ON DELETE / ON UPDATE | `['columns' => ['role_id'], 'references' => 'roles', 'on' => ['id'], 'on_delete' => 'set null', 'on_update' => 'cascade']` | Used in `Model::$foreignKeys` or as `Schema::createTableSql`/`foreignKeySql` argument. Actions: `cascade`, `set null`, `restrict`, `no action`, `set default` (case-insensitive). **Both `on_delete` and `on_update` are always emitted** — when omitted they default to `NO ACTION` so the generated SQL is always explicit |
...
| Cast model attributes | `protected static array $types = ['age' => 'int', 'price' => 'decimal:2', 'tags' => 'json', 'created_at' => 'datetime', 'status' => 'enum:' . Status::class]` | Applied on hydrate (read) and save (write); null passes through. See `Cloude\Model\Cast` for the type catalogue (`int`, `float`, `string`, `bool`, `decimal[:N]`, `json`/`array`, `datetime[:FMT]`, `date[:FMT]`, `enum:FQCN`) |
