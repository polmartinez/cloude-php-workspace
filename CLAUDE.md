...
| Cast Model attributes (typed read/write) | `protected static array $casts = ['age' => 'int', 'price' => 'decimal:2', 'tags' => 'json', 'created_at' => 'datetime', 'status' => 'enum:' . Status::class]` | Optional. Applied at hydrate (storage → PHP) and save (PHP → storage). Null always passes through. Catalogue: `int`, `float`, `string`, `bool`, `decimal[:N]`, `json`/`array`, `datetime[:FMT]`, `date[:FMT]`, `enum:FQCN`. See [`src/Model/Cast.php`](src/Model/Cast.php) |
...
| Emit CREATE TABLE from a model | Declare `$columns / $indexes / $foreignKeys` on the model, then `User::createTableSql('mysql')` / `User::dropTableSql()` | [`src/Storage/Schema.php`](src/Storage/Schema.php) — supports `mysql` + `pgsql` dialects. Not a migration runner — feed the output to phinx or pdo->exec() |
