...
| Send email (SMTP or sendmail) | `Mailer::forge()->send([...])` (reads `Config::get('email')`, falls back to legacy `'mail'` key) | [`examples/recipes/mail.php`](examples/recipes/mail.php), [`examples/recipes/config/email.php`](examples/recipes/config/email.php), [`src/Mail/`](src/Mail/) |
