...
| Emit CREATE TABLE SQL | `Schema::createTableSql($table, $columns, $indexes, $foreignKeys, 'mysql')` | [`src/Storage/Schema.php`](src/Storage/Schema.php) — standalone helper (not coupled to `Model`). Supports `mysql` + `pgsql`. Not a migration runner — feed the output to phinx or pdo->exec() |
