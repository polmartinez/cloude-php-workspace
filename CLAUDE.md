...
| Run N writes inside a transaction | `\Cloude\Storage\Transaction::run(fn() => …)` | Closure auto-commits / rolls back + rethrows. Manual form: `Transaction::begin()` / `commit()` / `rollback()` / `inTransaction()`. Nested calls use SAVEPOINTs |
...
| Sessions | Just use `Session::set/get/forget/all`; flash via `flash/pullFlash`; CSRF via `csrfToken/checkCsrf`; rotate ID on login with `regenerate()` | [`src/Session.php`](src/Session.php) — lazy auto-start on first access with hardened cookie defaults. Routes that never touch session never issue a cookie |
