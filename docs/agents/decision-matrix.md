...
| Upsert by lookup keys | `User::updateOrCreate(['email' => $e], ['name' => $n, 'active' => 1])` | Finds the row matching every `$match` k/v; updates with `$values` if present, creates `$match + $values` if missing (`$values` wins on overlap). `$values = []` makes it a read-or-create. **Not atomic alone** — pair with a UNIQUE index on `$match` columns + `Transaction::run()` for safe concurrent calls (`DuplicateKeyException` on the loser is recoverable) |
