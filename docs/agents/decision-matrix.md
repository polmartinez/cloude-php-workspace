...
| Auto-fill `created_at` / `updated_at` | Declare the columns in `$properties` — the rest is automatic | `Model::$timestamps` (bool, default `true`), `Model::$createdAt` / `$updatedAt` (column names, default `'created_at'`/`'updated_at'`). Columns NOT listed in `$properties` are silently skipped — opt-in by declaration |
